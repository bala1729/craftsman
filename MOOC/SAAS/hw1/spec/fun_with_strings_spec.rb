...
describe 'palindrome detection' do
  ...
  it 'should work for empty string' do
    ''.should_not be_a_palindrome
    nil.should_not be_a_palindrome
  end     
end
...
describe 'word count' do
  ...
  it 'ignores punctuation' do
    ...
    'A man\'s tried and '.count_words.should ==
      {'a' => 1, 'mans' => 1, 'tried' => 1, 'and' => 1}
  end
...
describe 'anagram grouping' do
  ...
  it 'Case insensitive "Scream cars for four scar creams"' do
    @anagrams =  'Scream cars for four scar creams'.anagram_groups
    @anagrams.each { |group| group.sort! }
    [%w(cars scar), %w(four), %w(for), %w(Scream creams)].each do |group|
        @anagrams.should include(group)
    end
  end  
end
